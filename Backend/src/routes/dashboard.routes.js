...
router.get("/stats", async (req, res) => {
  try {
    ...
    const byTypeMap = {};
    byType.forEach(item => {
      byTypeMap[item._id || "Unknown"] = item.count;
    });

    res.json({
      totalComplaints: total,
      resolvedComplaints: resolved,
      pendingComplaints: received,
      inProgressComplaints: in_review,
      byType: byTypeMap
    });
  } catch (err) {
    res.status(500).json({ msg: err.message || "Failed to fetch stats" });
  }
});
...
router.get("/complaints-over-time", async (req, res) => {
  try {
    ...
    res.status(500).json({ msg: err.message || "Failed to fetch data" });
  }
});
